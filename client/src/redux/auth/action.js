...
export const postRegister = (paload) =>(dispatch)=>{
    ...
    .then(res=>{
       return dispatch({type:types.POST_REGISTER_SUCCESS,paload:res.data})
    })
    .catch(err=>{
...
export const postLogin = (paload) =>(dispatch)=>{
    ...
    return axios.post(`${BASE_URL}/users/login`,paload)
    .then(res=>{
       return dispatch({type:types.POST_LOGIN_SUCCESS,paload:res.data})
    })
    .catch(err=>{
